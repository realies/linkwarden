...
import PasteUrlsImportModal from "./ModalContent/PasteUrlsImportModal";

type Props = {};

const ImportDropdown = ({}: Props) => {
  const { t } = useTranslation();
  const [pasteUrlsModal, setPasteUrlsModal] = useState(false);
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent side="bottom" align="start">
          <DropdownMenuItem onSelect={() => setPasteUrlsModal(true)}>
      ...
      {pasteUrlsModal && (
        <PasteUrlsImportModal onClose={() => setPasteUrlsModal(false)} />
      )}
    </>
  );
};
